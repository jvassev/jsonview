...
// The Addon SDK l10n library doesn't correctly locate locale files
//const _ = require("sdk/l10n").get;
const _ = require('./localization').get;
...
function JSONFormatter() {
  this.baseUrl = prefsService.get('extensions.jsonview@brh.numbera.com.sdk.baseURI', null);
}
...
JSONFormatter.prototype = {
  ...
  htmlEncode: function(t) {
    return (typeof t !== "undefined" && t !== null) ? t.toString().replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;") : '';
  },
  ...
  valueToHTML: function(value, path) {
    ...
    if (value === null) {
      ...
      return this.decorateWithSpan(value.slice(1), 'num');
    }
  ...
  arrayToHTML: function(json, path) {
    ...
    return (json.length !== 0 ? '<span class="collapser"></span>' : '') + '[<ul class="array collapsible">' + output + '</ul>]';
  },
  ...
  toHTML: function(content, title) {
    return '<!DOCTYPE html>\n' +
      ...
      '<link rel="stylesheet" type="text/css" href="' + this.baseUrl + "data/default.css" + '">' +
      '<script type="text/javascript" src="' + this.baseUrl + "data/default.js" + '"></script>' +
      '</head><body>' +
      content +
      '</body></html>';
  }
